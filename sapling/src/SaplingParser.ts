...
import { Tree, SaplingSettings, Token, ImportData } from './types';
...
export class SaplingParser {
   ...
   */
  public static parse(filePath: string, settings?: Partial<SaplingSettings>): Tree;
  public static parse(root: Tree, settings?: Partial<SaplingSettings>): void;
  public static parse(input: unknown, settings?: Partial<SaplingSettings>): unknown {
    const settingsObject = new SaplingSettings(settings);
    if (typeof input === 'string') {
      const entryFile = filePathFixer(input);
      ...
      ASTParser.parser(root, settingsObject);
      return root;
    }
    if (input instanceof Tree) {
      // ! returning undefined is necessary for in-place parsing to execute.
      return ASTParser.parser(input, settingsObject);
    }
...
const ASTParser = {
  ...
  parser(root: Tree, settings: SaplingSettings): void {
    const recurse = (componentTree: Tree): void => {
      ...
      if (!['\\', '/', '.'].includes(componentTree.importPath[0])) {
        ...
        const thirdParty = settings.aliases.every(
      ...
      if (!ASTParser.getFileName(componentTree, settings)) {
  ...
  getFileName(
    ...
    function closure(): string | null {
      ...
      if (settings.useAlias && componentTree.parentList.length) {
        ...
        if (!result.length && settings.webpackConfig && settings.wpResolver) {
          try {
            result = settings.wpResolver(settings.appRoot, componentTree.importPath) as string;
          } catch (err) {
        ...
        if (!result || path.basename(result) === `index.${path.extname(result)}`) {
          ...
          componentTree.set('error', 'Error when trying to find filepath for this component');
        }

        componentTree.set('filePath', result);
        return result;
      }
      ...
      try {
        ...
        const parsedFileName = fileArray.find((fileStr) => fileStr.match(regEx));
        if (parsedFileName) {
          const parsedFilePath = componentTree.filePath + path.extname(parsedFileName);
          componentTree.set('filePath', parsedFilePath);
          return parsedFilePath;
        }
        return null;
      } catch (err) {
        // File does not exist: Next.js component
        return componentTree.fileName;
      }
    }
  },
